...
**Evidence separation.** The single-replica contract is a CPU logical evaluator plus a mathematical ceiling over real artifacts. The structured recovery path is exercised on H200 and compared with a CPU oracle. The allocation result is a CPU/NumPy policy evaluation. These evidence classes answer different questions and are not merged into a single “H200 reliability” claim.
...
**Generality.** Four scientific fields support a scoped systems result, not universal workload generality. The NCU mechanism is shown on two fields. The depth sweep is synthetic. An optimized full-NMR implementation could differ from the measured 2.5× boundary.
...
## Appendix A. Scoped Logical Voting Controls

The only retained controls from the separate logical diversity matrix are:
...
These controls validate expected voting behavior; they are not main contributions. Temporal/ST error-reduction percentages are excluded from the abstract, contributions, RQs, figures, and conclusion.
